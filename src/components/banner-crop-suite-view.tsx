...
import { Crop, FileImage, GitBranch, Layers, Package, Zap } from 'lucide-react'
...
import BannerMasterWorkbenchView from '@/components/banner-master-workbench-v2-view'
...
export default function BannerCropSuiteView() {
  ...
  const activeLabel = mode === 'workbench'
    ? '母版工作台'
    : mode === 'pack'
      ? '旧版一键出包'
      : mode === 'masterPack'
  ...
  return (
    <Tabs value={mode} onValueChange={setMode} className="w-full">
      <div className="sticky top-0 z-20 border-b border-zinc-200/90 bg-[#fbfcff]/95 px-3 py-3 shadow-sm shadow-zinc-950/[0.03] backdrop-blur supports-[backdrop-filter]:bg-[#fbfcff]/80 sm:px-4 min-[1440px]:px-6">
        <div className="mx-auto flex w-full max-w-[1680px] flex-col gap-3 min-[920px]:flex-row min-[920px]:items-center min-[920px]:justify-between">
          ...
          <div className="min-w-0 overflow-x-auto pb-0.5">
            <TabsList className="h-10 w-max rounded-lg border-zinc-200 bg-zinc-100/80 p-1 shadow-inner shadow-zinc-950/[0.03]">
              <TabsTrigger value="workbench" className="h-8 gap-1.5 px-3 text-xs">
                <Layers className="h-3.5 w-3.5" />
                母版工作台
                <span className="rounded-sm bg-red-600 px-1 py-0 text-[9px] leading-4 text-white">推荐</span>
              </TabsTrigger>
